...
The domain access policy permits public data-plane reachability so managed Dashboards can proxy data requests. Fine-grained access control, HTTPS, and the generated internal admin credentials enforce authentication and authorization. The service is deliberately public for this short-lived learning environment because no bastion or VPN remains after removal of the EC2 stack.
...
- The endpoint is internet-reachable; protect the generated credentials and delete the stack when the exercise is complete.
